...
allocate_block(void)
{
	...
	for(i=0; i<ospfs_super->os_nblocks; i++){ //loop through bit map
...
add_block(ospfs_inode_t *oi)
{
	...
	if(n>OSPFS_MAXFILEBLKS)
		return -EIO;
	//need indirect1 block
	if(OSPFS_NDIRECT<=n && n-1 < OSPFS_NDIRECT){
	...
	if(n>=(OSPFS_NDIRECT+OSPFS_NINDIRECT) &&
	   (n-1) < (OSPFS_NDIRECT+OSPFS_NINDIRECT)) {
	...
	uint32_t* indir1_blk;
	uint32_t* indir2_blk;
	//if theres no indirect2 block
	if(indir2_in == -1) {
		memset(ospfs_block(direct), 0, OSPFS_BLKSIZE);
		//if theres also no indirect 1 block
		if(indir1_in == -1) {
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		// gets the size of the block you want to copy
		n = OSPFS_BLKSIZE - ((*f_pos) % OSPFS_BLKSIZE);
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	int err;
	if((err = change_size(dir_oi, entry)) < 0)
		return ERR_PTR(err);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	ospfs_direntry_t* dst_inode = create_blank_direntry(dir_inode);
	//check is destination inode is valid
	if(IS_ERR(dst_inode)) 
		return -EIO;
	//get information from source inode
	ospfs_inode_t* src_inode = ospfs_inode(src_dentry->d_inode->i_ino);
	// set destination inode to source inode
	dst_inode->od_ino = src_dentry->d_inode->i_ino;
	strncpy(dst_inode->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
	dst_inode->od_name[dst_dentry->d_name.len] = 0;
